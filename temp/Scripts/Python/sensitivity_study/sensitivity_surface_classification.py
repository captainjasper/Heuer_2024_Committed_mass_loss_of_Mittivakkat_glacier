...
path_cropped = "./Landsat/Sensitivity_study/Cropped/"
...
file_list = glob.glob("./Landsat/Sensitivity_study/Resampled/" + "*.tif", recursive=True)
...
for i in range(0, np.size(file_list)):
    img_in = file_list[i]
    img_out = "./Landsat/Sensitivity_study/Cropped/" + str(date_list[i]) + "_cropped.tif"
...
file_list2 = glob.glob("./Landsat/Sensitivity_study/Cropped/" + "*.tif", recursive=True)
...
for i in range(0, np.size(file_list2)):  
    ...
    reclass = np.where((band_01 > 0.50), 3,
    ...
    if no_data_ratio > (no_data_base_ratio + ((1 - no_data_base_ratio) * 0.2)): 
        ...
        with rasterio.open(
            "./Landsat/Sensitivity_study/Reclassified_05/" + str(date_list[i]) + "_reclass_05.tif",
            mode="w",
            driver="GTiff",
        ...
        with rasterio.open(
            "./Landsat/Sensitivity_study/Borders_05/" + str(date_list[i]) + "_borders_05.tif",
            mode="w",
            driver="GTiff",
            height=img_full.shape[1],
            width=img_full.shape[2],
